...
The next step would be to match WMEs (Working Memory Elements, ie, propositions) to rule arguments.  This is the point when we stopped and submitted our last paper, which was rejected.  We concluded that rules matching was too complicated and not worth the trouble. % and the traditional Transformer, as a distorted form of logic rules, somehow does the job equivalently, and may be even superior due to ``distributive representations.''
It turns out what was needed is the permutation of all WMEs, which is simply the Cartesian self-product $X^{\times n}$. % if we ignore ``replacement'' -- which is okay, if one doesn't mind the same WME matching more than one part of a rule.
This looks computationally prohibitive, but we're just back to the old problem faced by the Rete algorithm \cite{Forgy1982}, and there are many new (differentiable) ways to solve this problem. % The result is Fig. \ref{fig:logic-Transformer-string-diagram}.
...
\begin{longtable}{|p{2.5cm}|p{10cm}|}
	...
	\hline \textbf{Representation\newline of Working\newline Memory} & One of the key design choices of an AGI how to represent Working Memory.  OpenCog proposes a hypergraph structure. We explored this issue at length with Copilot and Qwen, particularly on trees because Chomsky revealed them to be a universal underlying structure of natural language. We looked at continuous representations of trees, whose algebraic structure is captured by \textbf{F-algebras}. Even so, faithfulness is very hard to guarantee in finite dimensional space. Many differentiable tree-rewriting techniques are complicated to implement.  We concluded that it is probably more efficient to use propositional logic rules for rewriting a graph-based Working Memory. \\
	...
	\hline \textbf{Reflection} & Refers to inference traces being made into inspectable objects fed into Working Memory. This allows an AI agent to examine its own thought processes. Some researchers regard this a core feature of ``consciousness.'' \\

	% \hline \textbf{Reification} & \\

	\hline \textbf{HOL} & Higher-Order Logic is needed to learn highly abstract rules that may achieve better compression. \\
